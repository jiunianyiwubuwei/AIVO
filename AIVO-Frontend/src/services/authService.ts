...
const normalizeUser = (raw: unknown): UserRespDTO | null => {
  ...
  const username = toString(raw.username ?? raw.user_name) || "";
  if (!username) return null;

  return {
    id: toNumber(raw.user_id ?? raw.id),
    username,
...
const extractUserFromAuthPayload = (payload: unknown): UserRespDTO | null => {
  ...
  if (isRecord(payload.data)) {
    const fromData = normalizeUser(payload.data);
...
const extractTokenFromAuthPayload = (payload: unknown): string | null => {
  ...
  const directToken = toString(
    payload.token ?? payload.accessToken ?? payload.access_token ?? payload.authToken,
  )?.trim();
  ...
  if (isRecord(nestedData)) {
    const nestedToken = toString(
      nestedData.token ?? nestedData.accessToken ?? nestedData.access_token ?? nestedData.authToken,
    )?.trim();
    if (nestedToken) return nestedToken;

    const nestedUser = normalizeUser(nestedData);
    if (nestedUser) {
      return null;
    }
  }
