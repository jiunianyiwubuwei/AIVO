...
export const aiService = {
  ...
  createAiModel: (data: CreateAiModelParams) => {
    return service.post("/xunzhi/v1/ai-properties", data);
  },

  updateAiModel: (data: UpdateAiModelParams) => {
    return service.put("/xunzhi/v1/ai-properties", data);
  },

  deleteAiModel: (id: number) => {
    return service.delete(`/xunzhi/v1/ai-properties/${id}`);
  },

  toggleAiModelStatus: (id: number, isEnabled: number) => {
    return service.put(`/xunzhi/v1/ai-properties/${id}/status`, null, {
      params: { isEnabled },
    });
  },

  deleteConversation: (sessionId: string) => {
    return service.delete(`/xunzhi/v1/ai/conversations/${encodeURIComponent(sessionId)}`);
  },
};
