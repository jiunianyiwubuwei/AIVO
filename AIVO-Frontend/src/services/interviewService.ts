...
const normalizeExtractInterviewQuestions = (
  ...
  return {
    ...
    resumeFilename:
      toStringValue(
        pickFirst(source, ["resumeFilename", "resume_filename"]),
...
const normalizeInterviewSessionRestore = (
  ...
  return {
    ...
    interviewDirection:
      toNullableString(
        pickFirst(source, ["interviewDirection", "interview_direction"]),
      ) ?? payload.interviewDirection,
    suggestions:
      toStringMap(
...
export const interviewService = {
  ...
  fetchInterviewResumePreviewBlob: async (sessionId: string) => {
    const path = buildResumePreviewPath(sessionId);
    const token = assertRequestAuthorized(path);
    console.debug("[InterviewService] Fetching preview blob for session:", sessionId, "URL:", path);
    ...
    console.debug("[InterviewService] Preview response status:", response.status, "Content-Type:", response.headers.get("content-type"));

    if (!response.ok) {
      const errorMessage = (await response.text()).trim();
      console.error("[InterviewService] Preview fetch failed:", response.status, errorMessage);
      throw new Error(
    ...
    console.debug("[InterviewService] Preview response size:", bytes.length, "bytes");
    ...
    if (!isPdf && !contentType.includes("pdf")) {
      const errorMsg = decodePreviewError(bytes) ||
        `Resume preview returned unexpected content type: ${contentType || "unknown"}`;
      console.error("[InterviewService] Not a valid PDF:", errorMsg, "First bytes:", Array.from(bytes.slice(0, 20)));
      throw new Error(errorMsg);
    }
    if (!isPdf) {
      const errorMsg = decodePreviewError(bytes) || "Resume preview is not a valid PDF";
      console.error("[InterviewService] PDF magic bytes missing:", errorMsg);
      throw new Error(errorMsg);
    }
    ...
    console.debug("[InterviewService] Preview blob created:", blob.size, "bytes");
    return blob;
  },
  uploadResume: async (params: UploadResumeParams) => {
    ...
    formData.append("biz_type", params.bizType || "resume");
  ...
  extractInterviewQuestions: async (
    ...
    console.log("[InterviewService] extractInterviewQuestions - file:", {
      name: params.resumePdf.name,
      size: params.resumePdf.size,
      type: params.resumePdf.type,
    });
  ...
  saveInterviewRecordFromRedis: async (sessionId: string) => {
    return postWithPathFallback<void>(
      ...
      `/xunzhi/v1/interview/save-from-redis/${encodeURIComponent(sessionId)}`,
    );
  },
  ...
  deleteInterviewRecord: async (sessionId: string) => {
    return service.delete(`/xunzhi/v1/interview/record/${encodeURIComponent(sessionId)}`);
  },
};
