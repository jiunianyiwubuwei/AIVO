...
const extractRadarPoints = (record: UnknownRecord | null): RadarPoint[] => {
  ...
  for (const key of candidateArrayKeys) {
    ...
    const parsed = raw
      .map(parseRadarPoint)
      .filter((item) => item !== null && item.value !== null) as RadarPoint[];
    if (parsed.length > 0) {
...
const buildRadarPointsFromDto = (
  ...
  // 优先使用 radarPoints 或 radarMetrics 数组
  const radarPointsArray = radar.radarPoints ?? radar.radarMetrics;
  if (Array.isArray(radarPointsArray) && radarPointsArray.length > 0) {
    const parsed = radarPointsArray
      .map((item) => {
        const label = typeof item === 'object' && item !== null
          ? ((item as Record<string, unknown>).label ?? (item as Record<string, unknown>).name ?? (item as Record<string, unknown>).dimension ?? (item as Record<string, unknown>).metric)
          : null;
        const value = typeof item === 'object' && item !== null
          ? ((item as Record<string, unknown>).value ?? (item as Record<string, unknown>).score ?? (item as Record<string, unknown>).percent)
          : null;
        const score = pickFirstNumber(value);
        if (!label || score === null) return null;
        return { label: String(label), value: score };
      })
      .filter((item) => item !== null && item.value !== null) as RadarPoint[];
  ...
  return metrics
    ...
    .filter((item) => item !== null) as RadarPoint[];
};
