...
export const mapBusinessCode = (code: string | number | undefined): ErrorCodeValue => {
...
export const createAxiosClient = (): AxiosInstance => {
  return axios.create({
    ...
    timeout: 10_000,
    // 不设置默认 Content-Type，让 axios 根据 data 类型自动选择
    // 当发送 FormData 时会自动设置为 multipart/form-data
  });
};
...
axiosInstance.interceptors.request.use(
  (config) => {
    console.log("[RequestInterceptor] URL:", config.url, "Method:", config.method, "Data type:", typeof config.data);
    if (config.data instanceof FormData) {
      console.log("[RequestInterceptor] FormData detected, entries:");
